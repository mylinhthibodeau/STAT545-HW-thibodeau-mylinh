...
You can do the same thing with other parameters, or in the case below, layering the gdp information in the same plot.
```{r}
p7 <- gapminder %>%
  ggplot(aes(x=year, y=lifeExp))
p7 + stat_summary_bin(mapping = aes(group = continent, colour=continent), fun.y = "median", geom="line") +
  scale_y_continuous(gapminder$gdpPercap, sec.axis = sec_axis(~./500))
```
