...
If we group data according to their column, another useful function is summarize() to get some statistics on a variable, but data needs to be grouped before doing so.
```{r}
overview <- gapminder %>%
  group_by_all() %>%
  summarise_all(funs(mean, median, sum, count=n()))
View(overview)
```



*Q? What values are typical? What’s the spread? What’s the distribution? Etc., tailored to the variable at hand.*  

*Q? Feel free to use summary stats, tables, figures. We’re NOT expecting high production value (yet).*  
