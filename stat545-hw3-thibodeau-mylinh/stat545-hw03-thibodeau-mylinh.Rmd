...
Here is a visual representation of the minimum and maximum value of RPKM for each cancer.gene.type.
[This tidyverse website article](http://ggplot2.tidyverse.org/reference/stat_summary.html) was very helpful.
...
p5 + geom_point(aes(colour=cancer.gene.type), alpha = 0.6) + 
  theme(text = element_text(size=12), axis.title.x=element_blank(), axis.text.x = element_blank(), axis.ticks.x=element_blank())
```
Much better ! So my understanding is that I have to treat the gene name (hugo) column as a factor and re-order this column according to the RPKM value if I want ascending RPKM values when I am ploting x=hugo and y=RPKM.
