...
* Life expectancy -> class numeric (e.g. avg.TCGA.percentile)
* Year -> chromosome (chr) or avg.cna.by.gene
...
**TABLE**
TO RE-DO
It is not that pretty, but the information is here
...
d3_low <- d3 %>%
  filter(avg.TCGA.percentile <=10) %>%
  group_by(cancer.gene.type) %>% 
  summarize(count_low=n()) 
d3_high <- d3 %>%
  filter(avg.TCGA.percentile >10) %>%
  group_by(cancer.gene.type) %>% 
  summarize(count_high=n())

left_join(d3_low, d3_high)  %>%
  kable("html") %>%
  kable_styling()
```
