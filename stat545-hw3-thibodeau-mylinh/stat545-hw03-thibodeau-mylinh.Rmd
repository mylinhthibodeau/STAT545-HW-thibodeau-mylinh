...
```{r}
rna_cnv_cancer_gene_subset <- with(rna_cnv, rna_cnv[!(cancer.gene.type ==""),])
...
```{r}
rna_cnv %>%
  filter(cancer.gene.type =="NA", RPKM == max(RPKM)) %>% kable("html") %>% kable_styling()
```
...
Hmm, I have had this problem in the past. I talked about it to Vincenzo Coia and he mentioned that I need to looking into factor and levels, so it took me a very very very long time to figure out the following:
...
d2 <- group_by(rna_cnv, hugo) %>%
  ...
  aggregate(avg.TCGA.percentile ~bin, data=d2, sum) %>%
d2 %>% 
  group_by(bin) %>%
...
I don't have a "time" value in my dataset, so again, I will use the avg.TCGA.percentile since it is an integer.
