...
library(scales)
#install.packages("tidygenomics") # which also installed fuzzyjoin packages
...
Getting the CHEK2 paper [Thibodeau et al. 2017](http://molecularcasestudies.cshlp.org/content/3/5/a001628.long) rna-seq and copy number variant (cnv) data and cleaning it up. 

*If you would like the details about the code chunk below, please refer to my [homework3](https://github.com/mylinhthibodeau/STAT545-HW-thibodeau-mylinh/tree/master/stat545-hw3-thibodeau-mylinh).*
...
Getting the irbesartan-colorectal cancer paper [Jones et al (2016)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4843189/) rna-seq and copy number variant (cnv) data and see what it looks like. 

```{r}
arb_rna_cnv <- read.table("/Users/mylinh/Documents/published_pog_data/stat545-tables/crc-arb-response-rna-cnv.txt", header = TRUE, sep="\t", fill = TRUE)
...
Tidyverse is an incredible package, and some scientists have expanded its concepts to genomics data. Constantine Ahlmann-Eltze's blog explained the use of the [Tidy Genomics package](https://cran.r-project.org/web/packages/tidygenomics/vignettes/intro.html), which I will be exploring during this homework. The peculiarity of genomics data is the need for accomodating ranges, and tidygenomics has its own functions for reshaping and aggregating data:
