...
```
    ...
    ## Joining, by = c("hugo", "cancer_type", "sample", "RPKM")
    ## Joining, by = c("hugo", "cancer_type", "sample", "RPKM")
    ## Joining, by = c("hugo", "cancer_type", "sample", "RPKM")
    ## Joining, by = c("hugo", "cancer_type", "sample", "RPKM")
    ## Joining, by = c("hugo", "cancer_type", "sample", "RPKM")
...
-   I grouped by hugo gene (year) and cancer\_type (continent)
...
-   Used spread() with a key = cancer\_type and a value=the\_number
-   Which means that the cancer\_type now became columns of their own
...
Well, I am not sure actually. There might be one: plotting the hugo (year) summary stats according to the cancer\_type (continent) will be easier I think.
...
full_dataset %>%
  ...
  ggplot(aes(x=hugo, y=RPKM, colour=cancer_type)) +
...
General data reshaping: Activity \#4 - Window functions
-------------------------------------------------------
...
Take that table (or a similar one for all cancer\_type) and reshape it so you have one row per year or per year \* continent combine
