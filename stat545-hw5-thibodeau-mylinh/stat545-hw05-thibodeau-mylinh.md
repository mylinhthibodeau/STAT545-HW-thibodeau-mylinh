...
``` r
levels(d3$cancer.gene.type) %>% kable("html") %>% kable_styling()
```
...
``` r
d3$cancer.gene.type %>% forcats::fct_infreq() %>% levels() %>% kable("html") %>% kable_styling()
```
...
levels(d3$avg.TCGA.percentile) %>% head(5) %>% kable("html") %>% kable_styling()
```
...
``` r
d3$avg.TCGA.percentile %>% forcats::fct_infreq() %>% levels() %>% head(5) %>% kable("html") %>% kable_styling()
```
...
``` r
d5.cancer.gene.type %>% kable("html") %>% kable_styling()
```
