...
This exercise is based on the GitHub tutorial [here](https://github.com/seandavi/GenomicDataCommons#filtering) which shows a step by step process to obtain some cancer genomic data.
...
library(GenomicDataCommons)
file_records = files() %>%
filter(~ cases.project.project_id == 'TCGA-DLBC' &
...
```{r}
d1 = cases() %>% filter(~ project.project_id=='TCGA-DLBC') %>%
